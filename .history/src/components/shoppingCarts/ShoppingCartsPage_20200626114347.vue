    ...
    <van-checkbox-group v-model="checkboxGroup" ref="checkboxGroup">
      <van-cell-group>
        <van-cell v-for="(item,index) in shopList" :key="index" clickable>
          <template #right-icon>
            <van-checkbox :name="item.name" ref="checkboxes" shape="square" />
          </template>
          <div class="img">
            <img :src="item.image_path" alt class="w100p" />
          </div>
          <div>                 <div class="color-red m-b-20 F18 m-l-10">{{item.name}}</div>                 <div class="color-red m-l-10 F18">￥{{item.mallPrice}}</div>               </div>
          <van-stepper
            v-model="item.count"
            theme="round"
            button-size="22"
