<template>
  <div>
    ...
    <van-card>
      ...
      <template #num>
        <div class="F16 m-l-10">{{lookItem.comment_time}}</div>
      </template>
      <template #footer>
        <div class="m-t-10 F16 m-l-10">评价内容:{{lookItem.content}}</div>
      </template>
      <template #desc>
        <van-rate
    ...
    <div class="flex p-10 a-center">
    <div class="img"><img :src="item.image" alt="" class="w100p"></div>
    <div class="m-l-10 w-o-t">{{item.name}}</div>
    <div class="carts flex a-center j-center"><van-icon name="shopping-cart-o" size="24px"  class="color-red"/></div>
    </div>
    </div>
  </div>
</template>
