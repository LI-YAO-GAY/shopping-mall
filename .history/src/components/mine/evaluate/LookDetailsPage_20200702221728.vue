<template>
  <div>
    ...
    <van-card>
      <template #thumb>
        <van-image round width="5rem" height="10rem" src="https://img.yzcdn.cn/vant/cat.jpeg" />
      </template>
      <template #title>
        <div class="m-t-10 F16 m-b-20 m-l-10" v-if="lookItem.anonymous===true">匿名用户</div>
      </template>
      <template #num>
        <div class="m-t-10 F16 m-b-20 m-l-10">{{lookItem.comment_time}}</div>
      </template>
      <template #footer>
        <div class="m-t-10 F16 m-b-20 m-l-10">评价内容:{{lookItem.content}}</div>
      </template>
      <template #desc>
        <van-rate
    ...
    <div v-for="(item,index) in lookItem.goods" :key="index"></div>
  </div>
</template>
