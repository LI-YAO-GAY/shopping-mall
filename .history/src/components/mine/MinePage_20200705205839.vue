        ...
        <div class="center">
          <div class="banner flex a-column a-center j-center">
            ...
            <van-image
              ...
              width="4rem"
              height="4rem"
              :src="require('../../assets/u=682748172,3788948432&fm=15&gp=0.jpg')"
              class="m-t-30"
            />
            <div v-if="user!==null">
              <div
          ...
          <div>
            <van-grid :column-num="5" :border="false">
              ...
              <div     v-if="TobeList.length>0&& user!==null">
              <van-grid-item
                icon="thumb-circle-o"
                text="评价"
                :badge="count"
              ...
              />
              <van-grid-item icon="thumb-circle-o" text="评价" @click="evaluate" v-if="user===null" />
              </div>
...
export default {
  ...
  methods: {
    ...
    collect() {
      this.$checkLogin(this.goCollect());
    },
    goAddress() {
      this.$router.push("/address");
    },
    address() {
      this.$checkLogin(this.goAddress());
    },
    goBrowse() {
      this.$router.push("/browse");
    },
    browse() {
      this.$checkLogin(this.goBrowse());
    },
    goEvaluate() {
      this.$router.push("/evaluate");
    },
    evaluate() {
      this.$checkLogin(this.goEvaluate());
    },
    goCompleted() {
      this.$router.push({ path: "/order", query: { index: 4 } });
    },
    completed() {
      this.$checkLogin(this.goCompleted());
    },
    changepersonal() {
