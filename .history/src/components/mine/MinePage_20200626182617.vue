    ...
    <div class="main">
      ...
      <div>
        <van-grid :column-num="5" :border="false">
          <van-grid-item icon="cash-back-record" text="待付款" dot @click="evaluate"/>
          <van-grid-item icon="free-postage" text="待发货" badge="99+" @click="evaluate"/>
          <van-grid-item icon="points" text="待收货" badge="99+" @click="evaluate"/>
          <van-grid-item icon="thumb-circle-o" text="评价" badge="99+" @click="evaluate"/>
          <van-grid-item icon="points" text="已完成" badge="99+" @click="evaluate"/>
        </van-grid>
      </div>

      <div class="flex a-center j-between p-10 all">
        <div class="flex a-center" @click="order">
          <van-icon name="records" class="m-r-5" />全部订单
        </div>
        <van-icon name="arrow" />
      </div>
      <div class="flex a-center j-between p-10 all m-t-20">
        <div class="flex a-center" @click="collect">
          <van-icon name="star-o" class="m-r-5" />收藏商品
        </div>
        <van-icon name="arrow" />
      </div>
      <div class="flex a-center j-between p-10 all" >
        <div class="flex a-center" @click="address">
          <van-icon name="location-o" class="m-r-5" />地址管理
        </div>
        <van-icon name="arrow" />
      </div>
      <div class="flex a-center j-between p-10 all">
        <div class="flex a-center" @click="browse">
...
export default {
  ...
  methods: {
    ...
    order(){this.$router.push('/order')},
    collect(){this.$router.push('/collect')},
    address(){this.$router.push('/address')},
    browse(){this.$router.push('/browse')},
    browse(){this.$router.push('/browse')},
    browse(){this.$router.push('/browse')},
    browse(){this.$router.push('/browse')},
  },
