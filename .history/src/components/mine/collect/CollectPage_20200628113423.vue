    ...
    <div class="content">
      <div v-for="(item,index)  in list" :key="index">
        <div class="flex m-10 a-center">
          ...
          <div class="icon" @click="del(item,index)"><van-icon name="close"/></div>
        </div>
        <div class="line" v-if="index!=list.length-1"></div>
      </div>
      <div class="flex a-center j-center" v-if="list.length === 0">暂无收藏</div>
    </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.$api
      ...
      // this.listArr=JSON.parse(localStorage.getItem('collectGoods'))
      // this.list.push(this.listArr)
