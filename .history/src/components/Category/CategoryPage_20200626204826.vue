    ...
    <div class="tab" ref="tab">
        ...
        <div class="flex main-bottom">
            ...
            <div class="flex right" v-if="categorydata.length>0">
                <div v-for="(item1,index1) in categorydata[0].bxMallSubDto" :key="index1" class="right-nav">
                    <van-tabs swipeable :scrollspy="true">
                        <van-tab :title="item1.mallSubName"></van-tab>
                        <div>{{item1.mallSubId}}</div>
                    </van-tabs>
                </div>
            </div>
...
export default {
    ...
    mounted() {
        this.$api.getClassification(this.categorydata).then(res=>{
         console.log(res);
         this.dataList = res
        }).catch(err=>{
          console.log(err)
        ...
        // console.log(this.categorydata[0].bxMallSubDto.mallSubId);
    },
    watch: {},
    computed: {
        // CategoryData() {
