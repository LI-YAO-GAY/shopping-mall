        ...
        <div class="flex main-bottom j-between">
            ...
            <div class="flex right" v-if="categorydata.length>0">
                <van-tabs v-model="active" swipeable @click="onClick">
                    <van-tab v-for="(item1,aIndex) in categorydata[aIndex].bxMallSubDto" :key="aIndex" :title="item1.mallSubName" ellipsis="false">
                        <div>
...
export default {
    ...
    methods: {
        ...
        OnChange(item, index) {
            ...
            //我是用来绑定类名的，嗯嗯，你也可以用来取数据，点击左边过后比如第3个，
           // 然后页面就会显示数组的第三项里面的第一个
           //换成代码就是这样表示
          // 所以你点击左边的哪个，num就是几,然后点击左边过后，它默认显示的是右边的第一个item
          //this.categorydata[this.num].bxMallSubDto[0].mallSubId;嗯
