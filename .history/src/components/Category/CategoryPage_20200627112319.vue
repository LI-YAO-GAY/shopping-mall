    ...
    <div class="tab" ref="tab">
      ...
      <div class="flex main-bottom j-between">
        <div class="flex a-column left a-center">
          <div v-for="(item,index) in categorydata" :key="index" class="p-10 left-item">
            <input type="button" :value="item.mallCategoryName" class="ipt" @click="OnChange(item,index)" :class="{'white' : num === 0}"/>
          </div>
        </div>
        <div class="flex right" v-if="categorydata.length>0">
          <van-tabs v-model="active" swipeable @click="onClick">
            <van-tab
              v-for="(item1,aIndex) in categorydata[aIndex].bxMallSubDto"
              :key="aIndex"
