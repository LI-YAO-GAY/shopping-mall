  ...
  <div>
    ...
    <div class="top">
      <van-cell-group class="flex">
        <van-icon name="location-o" size="24px" class="m-t-20 m-l-10" />
        <van-cell>
          <template #title>
            <div class="F18">收货人:{{defaultAdd.name}}</div>
          </template>
          <template #default>
            <div class="F14">内容</div>
          </template>
          <template #label>
            <div class="F14">收货地址:</div>
            <div class="F14 toast">(收货不便时，可选择免费代收货服务)</div>
          </template>
        </van-cell>
      </van-cell-group>
