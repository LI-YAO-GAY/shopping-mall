  ...
  <div>
    ...
    <div>
      <div v-for="(item,index) in arr" :key="index">
        <div class="flex line" @click="Godetails(item)">
          <div class="m-l-10 img m-tb-10">
            <img :src="item.image_path" class="w100p" />
          </div>
          <div class="m-tb-10">
            <div class="m-b-20 m-l-10 name">{{item.name}}</div>
            <span class="color-red m-lr-10">￥{{item.orl_price}}</span>
            <span class="F14 zprice color-ccc">{{item.present_price}}</span>
          </div>
        </div>
      </div>
    </div>
