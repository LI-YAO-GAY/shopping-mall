<template>
  <div>
    ...
    <van-submit-bar :price="total1" button-text="提交订单" @submit="onSubmit" />
  </div>
</template>
      ...
      if (this.$route.query.arr) {
        ...
        this.$api
          ...
          .then(res => {
            ...
            this.arr = [];
            localStorage.setItem("shopping", JSON.parse(this.arr));
            this.$router.go(-1);
          })
          .catch(err => {
            console.log(err);
        ...
        this.$api
          ...
          .then(res => {
            ...
            this.arr1 = [];
            localStorage.setItem("details", JSON.parse(this.arr1));
            this.$router.go(-1);
          })
          .catch(err => {
            console.log(err);
