      ...
      <div class="main_left">
        ...
        <div>
          <div v-if="flag === true" class="history">
            ...
            <div v-for="item in historyArr" :key="item.id" class="F14">
...
export default {
  ...
  methods: {
    Focus() {
      ...
      this.flag = true;
      let historyArr = JSON.parse(localStorage.getItem("searchValue"));
    ...
    onSearch(val) {
      ...
      this.$api
        ...
        .then(res => {
          ...
          this.history = this.history.push(localStorage.setItem("searchValue", this.value))
          let historyArr=[]
          this.historyArr =  this.history
          // this.historyArr.push(localStorage.setItem("searchValue", this.value))
          localStorage.setItem("searchValue", JSON.stringify(historyArr))
          this.historyArr = this.history.filter(function(item,index,array){
    ...
    name(value, keyword) {                
      ...
      if (value) {                   //如果有输入的值
        let res = value.replace(    //定义一个变量====输入的值替换为关键字，替换的值
          Reg,`<span style="color:red">${keyword}</span>`
        );
        return res;                //返回一个res
      }
    }
