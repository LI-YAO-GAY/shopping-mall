<template>
  <div>
    ...
    <van-search v-model="serchCity" placeholder="请输入城市关键词" @input="OnSearch" @focus="Focus"/>
    ...
    <div class="flex f-w m-b-10">
      <div v-for="(item,index) in hotCities" :key="index">
        <div class="hotCity flex a-center j-center F14 f1 m-l-15 m-t-10" @click="checkCity(item)">{{item.name}}</div>
      </div>
    </div>
    ...
    <div v-if="flag===true">
    <div class="bg-999 searchPage"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    checkCity(item1) {
      this.serchCity = item1.name;
      console.log(item1.name);
      localStorage.setItem('serchCity',this.serchCity)
      this.$router.push({path:'/',query:{serchCity:this.value}})
    },
    OnSearch(val) {
       this.flag = false
      console.log(this.serchCity);
      // let arr = [];
      // let searchArr = [];
      // if (this.serchCity == "") {
