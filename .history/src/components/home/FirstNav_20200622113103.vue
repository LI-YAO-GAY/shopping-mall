...
export default {
  ...
  methods: {
    Focus() {   //定义一个中间变量-默认不显示flag=false，获取焦点=>flag=true显示搜索历史界面，=>flag=false搜索历史界面隐藏，显示列表界面
      this.flag = true;  
      let history = JSON.parse.localStorage.getItem(this.value);
    },
    cancel(){            //如果没有数组或者获取焦点时，显示取消按钮
      this.flag = false; //点击取消的时候，失去焦点flag=false ,清空数组，清空搜索的内容
      this.list=[];      
      this.value=""
    },
    onSearch(val) {     //如果存在数组(length=0)并且没有获取焦点时，显示搜索按钮
      this.$api   
