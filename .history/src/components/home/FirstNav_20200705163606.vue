      ...
      <div class="main_left">
        <form action="/">
          ...
          >
            ...
            <template #left>
            <div @click="onCity" class="F14 m-l-5" v-if="position_city">{{position_city}}</div>
              <div @click="onCity" class="F14 m-l-5" v-else>{{city}}</div>
              
              <van-icon name="arrow-down" class="m-r-5 F12" @click="onCity" />
            </template>
          </van-search>
        </form>
        <div>
          <div v-if="flag === true" class="history">
            <div class="p-10 h-bottom flex j-between">
              <div>搜索历史</div>
              <div @click="clearHistoryAll">
                <van-icon name="delete" size="20px" />
              </div>
            </div>
            <div class="flex f-w">
              <div v-for="item in historyList" :key="item.id" class="F14 m-10">
                <span class="item" @click="searchHistory(item)">{{item}}</span>
              </div>
            </div>
          </div>
          <div class="list" v-if="flag===false">
            <div v-for="(item,index) in list" :key="index">
              <!-- 关键字高亮-->
              <div
                class="list-item p-tb-10 bg-fff p-l-10"
                v-html="name(item.name,value)"
                @click="searchHistoryDetails(item)"
              ></div>
            </div>
          </div>
        </div>
...
export default {
        ...
        .then(res => {
          ...
          if (this.searchValue !== "") {
            if (localStorage.getItem("searchValue")) {
              ...
              localStorage.setItem("searchValue", JSON.stringify(history));
            } else {
              let arr = [];
              arr.push(this.value);
  ...
  mounted() {
    ...
    this.historyList = uniqWith(this.historyArr, isEqual);
    if (localStorage.serchCity) {
      this.position_city = localStorage.getItem("serchCity");
    } else {
      this.getSomeCity()
    }
  },
