    ...
    <div class="main flex a-center j-around p-tb-10">
      <div class="main_left">
        <form action="/">
          ...
          >
            <template #action>
              <div @click="onSearch" v-if="flag===false && list.length===0">搜索</div>
              <div @click="cancel"   v-if="flag===true || list.length > 0 ">取消</div>
            </template>
            <template #left>
              <div @click="onCity" class="F14">{{city}}</div>

              <van-icon name="arrow-down" class="m-r-5 F14" />
            </template>
          </van-search>
        </form>
