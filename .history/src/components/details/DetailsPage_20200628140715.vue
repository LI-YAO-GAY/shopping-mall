  ...
  <div>
    ...
    <van-popup v-model="show" closeable position="bottom" close-icon="close">
      <div class="content">
        ...
        <div class="Buy p-tb-10 flex j-center color-fff m-t-40" @click="onBuyClicked">确定</div>
      </div>
    </van-popup>
  </div>
</template>
    ...
    getGoodsDetails() {
      this.$api
        ...
        .then(res => {
          ...
          localStorage.setItem("browseList", JSON.stringify(this.obj));
          let browse = localStorage.setItem("browseList", JSON.stringify(this.obj));
          this.browse.push(browse)
          // this.browseArr = this.history.filter((item, index, array) => {
          //   return array.indexOf(item) === index;
          // });
           localStorage.setItem("browseList", JSON.stringify(this.browseArr));
           console.log(this.browseArr);
        })
        .catch(err => {
          console.log(err);
        });
