<template>
  <div>
    ...
    <van-goods-action>
      ...
      <van-goods-action-button type="warning" text="立即购买" @click="onAddCartClicked" />  <!--写反了-->
      <van-goods-action-button type="danger" text="加入购物车" @click="buyNow" />
    </van-goods-action>
...
export default {
  ...
  methods: {
    ...
    //加入购物车
    buyNow() {
      this.$checkLogin(this.shoppingPayment); //有错！！！
      Toast({
        icon: "success",
    ...
    changeCount(){
       this.$api
        .getEditCart({count:this.count,id:this.id,mallPrice:this.mallPrice})
        .then(res => {
          console.log(res)
        })
        .catch(err => {
