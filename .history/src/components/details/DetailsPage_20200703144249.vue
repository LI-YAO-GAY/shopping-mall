          ...
          <div>
            <div v-for="(item3,index3) in evaluateItem" :key="index3">
              <van-card class="relative">
                ...
                <template #footer>
                  <div class="F14 t-align-l w-o-t MsgContent" v-if="evaluateItem.length===0">评价内容:{{item3.content}}</div>
                  <div class="F14 t-align-l w-o-t MsgContent" v-if="evaluateItem.length===0">暂无评价</div>
                </template>
              </van-card>
            </div>
          </div>
