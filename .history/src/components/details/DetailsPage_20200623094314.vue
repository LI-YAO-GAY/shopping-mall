  ...
  <div>
    <div class="main flex a-column">
      ...
      <div class="flex j-between font F14 p-lr-10 p-tb-10">
        ...
        <div>
        <div class="flex a-center" v-if="flag === false">
          收藏:
          <van-icon name="like"  class="m-r-10" @click="collect" :class="{'red' : flag===true}"/>
        </div>
        <div class="flex a-center" v-if="flag === true">
          取消收藏:
          <van-icon name="like"  class="m-r-10" @click="CancelCollect" :class="{'red' : flag===true}"/>
        </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    collect(){
      this.flag = true
      this.$api.getCollection(this.goods).then(res=>{
        console.log(res);
      }).catch(err=>{
        console.log(err);
      })
    ...
    CancelCollect(){
      this.$api.getCancelCollection(this.goods).then(res=>{
        console.log(res);
      }).catch(err=>{
        console.log(err);
      })
