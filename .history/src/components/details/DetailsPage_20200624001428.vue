<template>
  <div>
    ...
    <van-goods-action>
      ...
      <van-goods-action-button type="warning" text="加入购物车"  @add-cart="onAddCartClicked"/>
      <van-goods-action-button type="danger" text="立即购买" />
    </van-goods-action>
      <van-action-sheet v-model="show" v-if="flag===true"  :quota="quota">
        <div class="content">
          <div class="flex">
            <div class="shett-img">
              <img :src="obj.image" alt class="w100p" />
          ...
          <div>购买数量:</div>
          <div>剩余:{{obj.amount}}</div>
          <div class="Buy p-tb-10 flex j-center color-fff m-t-20"  @buy-clicked="onBuyClicked">立即购买</div>
        </div>
      </van-action-sheet>
  </div>
</template>
